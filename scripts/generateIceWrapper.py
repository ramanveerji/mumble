...
def generateFunction(className, functionName, wrapArgs, callArgs):
    function = (
        f"void ::MumbleServer::{className}I::{functionName}_async("
        + (", ".join(wrapArgs))
        + ") {\n"
    )
    function += "\t// qWarning() << \"" + functionName + "\" << meta->mp.qsIceSecretRead.isNull() << meta->mp.qsIceSecretRead.isEmpty();\n"
    function += f"#ifndef ACCESS_{className}_{functionName}" + "_ALL\n"
    ...
    function += "\t\t}\n"
    function += "\t}\n"
    function += f"#endif // ACCESS_{className}_{functionName}" + "_ALL\n"
    function += "\n"
...
def main():
    ...
    for currentLine in generatedIceHeader.split("\n"):
        ...
        if match := re.match(
            "^class\s+AMD_(.+)\s+:\s+(?:public\svirtual|virtual\s+public)\s+::Ice(?:::AMDCallback|Util::Shared)",
            currentLine,
        ):
            className = f"AMD_{match[1]}"

        if not className:
            if match := re.match(
                "virtual\s+void\s+ice_response\\((.*)\\)\s+=\s+0;", currentLine
            ):
                raise RuntimeError("Expected a className to be found at this time")
        if match := re.match(
            "virtual\s+void\s+(.+)_async\(const\s+(.+?)&\s*\w*,(.*)\s+const\s+::Ice::Current&",
            currentLine,
        ):
            ...
            wrapArgs = [f"const {objectName} &cb"]
            callArgs = ["cb"]
